...
\par The \inlinecode{cullMode} variable determines the type of face culling to use. One could disable culling, cull the front faces, cull the back faces or both. \inlinecode{frontFace} variable specifies the vertex order for faces to be considered front-facing and can be clockwise or counterclockwise.
...
\par If using a depth or stencil buffer, then you need to configure the depth and stencil tests using \inlinecode{VkPipelineDepthStencilStateCreateInfo}. Since we don't have any of this, we can just pass a nullptr to this.
...
\par There are two structs for color blending. The first: \inlinecode{VkPipelineColorBlendAttachmentState} contains the configuration per attached framebuffer. The second: \inlinecode{VkPipelineColorBlendStateCreateInfo} contains the global color blending settings. Since we only have one framebuffer:
...
\par This per-framebuffer stuct allows configuring the first way of color blending. If \inlinecode{blendEnable} is set to \inlinecode{VK\_FALSE}, then the new color from the fragment shader is passed through unmodified. Otherwise, the two mixing operations are perfomed to compute a new color. The resultant color is 'and'd' with the \inlinecode{colorWriteMask} to determine which channels are actually passed through.

\par The most common implementation of color blending is alpha blending, where we want the new color to be blended with the old on its opacity. We can implement all this as:
...
\par All possible operations in the \inlinecode{VkBlendFactor} and \inlinecode{VkBlendOp} enumerations in the specification.
...
\par If you want to use the second method of blending(bitwise), then you should set \inlinecode{logicOpEnable} to \inlinecode{VK\_TRUE}. The bitwise operation can then be specified in the \inlinecode{logicOp} field. This will automatically disable the first method. The \inlinecode{colorWriteMask} will also be used in the mod to determine which channels in the framebuffer will actually be affected.

\par One can use the \inlinecode{uniform} values in shaders, which are globals similar to dynamic state variables that can be changed at drawing time to alter behavior of your shaders withou having to recreate them. They are commonly used to pas the transformation matrix to the vertex shader, or to create texture samplers in the fragment shader. These uniform values need to be specifed during pipeline creation by creating a \inlinecode{VkPipelineLayout} object. Even though we aren't going to use them yet. However we still need to create an empty pipeline layout. So we need to create a private class member for this object: \inlinecode{VkPipelineLayout pipelineLayout;}. Then we need to create the object in \inlinecode{createGraphicsPipeline} function:
...
\par We need to tell Vulkan about the framebuffer attachments that will be used whilst rendering. We need to specify how many color and depth buffers there will be, how many samples for each of them and how their contents should be handled throughout rendering operations. This is all wrapped in a render pass object. We will create a new \inlinecode{createRenderPass} function and call this BEFORE the \inlinecode{createGraphicsPipeline}.
...
\par The \inlinecode{format} of the color attachment should match the format of the swap chain images, and since we are doing multisampling yet, we will do only $1$ sample:
...
\par The \inlinecode{loadOp} and \inlinecode{storeOp} apply to color and depth data, and \inlinecode{stencilLoadOp}/\inlinecode{stencilStoreOp} apply to stencil data. So the results of loading and storing are irrelevant:
...
\par Textures and framebuffers in Vulkan are represented by \inlinecode{VkImage} objects with certain pixel format, however the layout of the pixels in memory can change based on what you are trying to do with an image:

\begin{itemize}
    \item \inlinecode{VK\_IMAGE\_LAYOUT\_COLOR\_ATTACHMENT\_OPTIMAL} images are used as color attachment
    \item \inlinecode{VK\_IMAGE\_LAYOUT\_PRESENT\_SRC\_KHR} imagess are to be presented to the swap chain
    \item \inlinecode{VK\_IMAGE\_LAYOUT\_TRANSFER\_DST\_OPTIMAL} images to be used as destination for a memory copy operation.
\end{itemize}
...
\par The \inlinecode{attachment} parameter specifies which attachment to reference by its index in the attachment descriptions array. This array is going to consist of a single \inlinecode{VkAttachmentDescription} so the index is $0$. The \inlinecode{layout} tells which layout we want the attachment to have during a subpass utilizing this reference. Vulkan will automatically transition the attachment to this layout when the subpass is started. We intend to use the attachment to function as a color buffer and the \inlinecode{VK\_IMAGE\_LAYOUT\_COLOR\_ATTACHMENT\_OPTIMAL} layout will give the best performance for now. 

\par The subpass is described using a \inlinecode{VkSubpassDescription} structure. Vulkan may also suppport compute subpasses in the future, so we have to be explicit about this being a graphics subpass. Now we specify the reference to the color attachment:
...
\par The index of the attachment in this array is directly referenced from the fragment shader with the \inlinecode{layout(location = 0) out vec4 outColor} directive. These $4$ types of attachments can be referenced by the subpass:

\begin{itemize}
    \item \inlinecode{pInputAttachments} Attachments read from a shader
    \item \inlinecode{pResolveAttachments} Attachments used for multisampling color attachments
    \item \inlinecode{pDepthStencilAttachment} Attachment for depth and stencil data
    \item \inlinecode{pPreserveAttachments} Attachments that are not used by this subpass, but for which the data must be preserved.
\end{itemize}

\par Now that the attachment and a basic subpass referencing it have been described, we can creat the render pass now. Create a new pricate class member variable holding the \inlinecode{VkRenderPass} oject right aboce the \inlinecode{piplienlayout} variable.
...
\par We can then create the render pass object by filling in \inlinecode{VkRenderPassCreateInfo} structure witha n array of attachments ad subpasses. The \inlinecode{VkAttachmentReference} objects reference attachmetns using the indices of this array.
...
\par We can now combine all that crazy buildup fo shader stages, fixed function state, pipeline layout, and render pass. These all define the functionality of the graphics pipeline so we can start will the \inlinecode{VkGraphicsPipelineCreateInfo} structure at the end fo \inlinecode{createGraphicsPipeline} function. Yet before the calsto \inlinecode{vkDestroyShaderModule} because these sill need to be used during the creation.
...
\par We start by referenceing the array of \inlinecode{VkPipelineShaderStageCreateInfo} structs.
...
\par This means we finally ahve the reference to the render pass and the index of the sub pass where this graphics pipeline is used.  It is possible to use other render passes with this pipeline isntead of another, but they have to be compatible with \inlinecode{renderPass}. Here is some code about that:
...
\par There are actually two more parameters: basePipelineHandle and \inlinecode{basePipelineIndex} Vulkan allows one to create a new graphics pipeline by deriving from an existing pipeline. The core idea is that it is less expensive to set up pipelines with the same functionality with an existing pipeline and switching between pipelines from the same parent can be done quicker.One can specify then handle of an existing pipeline with \inlinecode{basePipelineHandle} or reference another pipeline about to be create by index with \inlinecode{basePipelineIndex}. Right now there is only a single pipeline so we will specify a null handle and an invalid index. These are only used if the \inlinecode{VK\_PIPELINE\_CREATE\_DERIVATIVE\_BIT} flag is also specifed in the flags field of \inelinecode{VkGraphicsPipelineCreateInfo}. Create a \inlinecode{VkPipeline} object.
...
\par The \inlinecdoe{vkCreateGraphicsPipeline} function actually has more parameters than the usual object creation functions in vulkan. It is designed to take multiple \inlinecode{VkGraphicsPipelineCreateInfo} objects and create mulitiple \inlinecode{VkPipeline} in a single call.

\par the second parameter, for which we passed the null handle argument, references an optional \inlinecode{VkPipelineCache} object. This can be used to store and reusue data relevant to pipeline creation across multiple calls to \inlinecode{vkCreateGraphicsPipelines} and even across program executions if the cache is stored to a file. Since the pipeline is used for all common drawing operations, it should be destroyed at the end of the program:
...
\par The attachments we specifed during the render pass creation are bound by wrapping them into a \inlinecode{VkFramebuffer} object. A framebuffer object references all of the \inlinecodeP{VkImageView} objects representing the attachments. In the case of the triangle, there is only one: the color attachment.
...
\par We will create the objects for this array in a new \inlinecode{createFramebuffers} function called after creating the graphics pipeline in \inlinecode{initVulkan}. We will start out the function by resizing the container to hold all the framebuffers. Then we will iterate through the image views and craete framebuffers from them.
...
\par You first need to specify with which \inlinecode{renderPass} the framebuffer needs to be compatible with. This is a requirement.

\par the \inlinecode{attachmentCount} and \inlinecode{pAttachments} parameters specify the \inlinecode{VkImageView} objects that should be bound to the respective attachment descriptions in the render pass \inlinecode{pAttachment} array. The width and height parameters explain themselves and layers refeters to the number of layers in image arrays. Our swap chain images are single images so the number of layers is $1$. We shoud delete the framebuffesr before the image views and render pass they are based on only after finishin rendering. (note since in a vector need to loop).
...
\par We have to create a command pool before we can create command buffers. Command pools manage the memroy that is used to store the buffers and command buffers are allocated from them. Add another class member by the name of \inlinecode{VkCommandPool}. Create a new function by the name of \inlinecode{createCommandPool} and call it in \inlinecode{initVulkan} after the framebuffers were created. Command pool creation only takes two parameters:
...
\par We will be recording a command buffer ever frame so we want to be able to reset an rerecord over it thus we will use the second flag. Command buffers are executed by submitting them on one of the device queues, like the graphics and presentation queues we retrived. Each command pool can only allocate command buffers that are submitted on a single type of queue. We are going to record commands fro drawing so this i swhy we chose the graphics queue family:
...
\par We can now start our allocation of command buffers. Create a \inlinecode{VkCommandBuffer} object class member and call it \inlinecode{commandBuffer}. These are automatically freed when their command pool is destroyed so no explicit cleanup is needed. You can add the function at the bottom of the call stack in \inlinecode{initVulkan}.

\par Command buffers are allocated with the \inlinecode{vkAllocateCommandBuffers} function, which takes a \inlinecode{VkCommandBufferAllocateInfo} struct as a parameter that specifies the commadn pool and number of buffers to allocate:
...
\par The level parameter specifies if the allocated command buffers are primary or secondary command buffers.

\begin{itemize}
    \item \inlinecode{VK\_COMMAND\_BUFFER\_LEVEL\_PRIMARY} Can be submitted to a queue for execution, but cannot be called from other command buffers.
    \item \inlinecode{VK\_COMMAND\_BUFFER\_LEVEL\_SECONDARY} Cannot be submitted directly, but can be called from primary command buffers/
\end{itemize}

\par For now we are not going to use the secondary command buffer. Since we are only allocating one command buffer, the \inlinecode{commandBufferCoutn} parameter is just $1$.

\par We will now move to \inlinecode{recordCommandBuffer} functionat that writes commands we want to execute into a command buffer. The \inlinecode{VkCommandBuffer} used will be passed in as a parameter as well as the inex of the current swap chain image we want to write to.
...
\par We always begin recording a command buffer by calling \inlinecode{vkBeginCommandBuffer} with a small \inlinecode{VkCommandBufferBeginInfo} structure as argument that specifies some details about the usage of  a particular command buffer:
...
\par the flags parameter specifies how we are going to use the command buffer. These are the possible values:

\begin{itemize}
    \item \inlinecode{VK\_COMMAND\_BUFFER\_USAGE\_ONE\_TIME\_SUBMIT\_BIT} The commadn buffer will be rerecorded after executing it once.
    \item \inlinecode{VK\_COMMAND\_BUFFER\_USAGE\_RENDER\_PASS\_CONTINUE\_BIT} This is a secondary command buffer that wil be entirely within a single render pass
    \item \inlinecode{VK\_COMMAND\_BUFFER\_USAGE\_SIMULTANEOUS\_USE\_BIT} The command buffer can be resubmitted while it is also already pending execution
\end{itemize}

\par We won't use any of these right now. The \inlinecode{pInheritanceInfo} parameter is only relevant for secondary command buffers. It specifies which state to inherit from the calling primary command buffers.

\par If the comand buffer has already been recorded, then call \inlinecode{vkBeginCommandBuffer} will reset it. It is not possible to append commands to a buffer at a later time. 

\par Drawing starts by starting the render pass with \inlinecode{vkCmdBeginRenderPass}. The render pass is configured using som parameters in a \inlinecode{VkRenderPassBeginInfo} struct.
...
\par The next two parametesr define the size of the render area. The render area then defines where shader loads and stores will take place. The pixels outside this region will have undefined values. For the best performance, it should match the seize of the attachments.
...
\par The last two parameters define the vlear values to use for \inlinecode{VK\_ATTACHMENT\_LOAD\_OP\_CLEAR}, which we used as a load operation for the color attachment. In this case we made it black with 100\% opacity:
...
\par The render pass can now start. All functions recording commands can be ided by their \inlinecode{vkCmd} prefix.
...
\begin{itemize}
    \item \inlinecode{VK\_SUBPASS\_CONTENTS\_INLINE} The render pass commands will be embedded in the primary command buffer itself and no seconday command buffers are executed.
    \item \inlinecode{VK\_SUBPASS\_CONTENTS\_SECONDARY\_COMMAND\_BUFFERS} The render pass commands will be executed from secondary command buffers.
\end{itemize}

\par For now, we aren't using secondary command buffers, so we will do the first option.
...
\par The actual \inlinecode{vkCmdDraw} function ain't much but its simple from all the boilerplate. it has these parameters:

\begin{itemize}
    \item \inlinecode{vertexCount} Even though we don't have a vetex buffer, we technically still have to draw 3 vertices.
    \item \inlinecode{instanceCount} Used for instanced rendering, use $1$ if you are not doing that.
    \item \inlinecode{firstVertex} Used as an offset into the vertex buffer, defines the lowest value of \inlinecode{gl\_VertexIndex}
    \item \inlinecode{firstInstance} Used as an offset for instanced rendering, defines the lowest \inlinecode{gl\_InstanceIndex}.
\end{itemize}
